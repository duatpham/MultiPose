...
def openP(fil,videonum,framenum, nam):
    ...
    while doc == 0:
        doc += 1
        frameWidth = fil.shape[1]
        frameHeight = fil.shape[0]
        inp = cv.dnn.blobFromImage(fil, inScale, (inWidth, inHeight),(0, 0, 0), swapRB=False, crop=False)
        net.setInput(inp)
        out = net.forward()
        ...
        for i in range(len(BODY_PARTS)):
            for j in range(len(detected_keypoints[i])):
                ...
                cv.ellipse(fil,detected_keypoints[i][j][0:2],(3, 3), 0, 0, 360, colors[i], cv.FILLED)
        ...
        for i in range(17):
            for n in range(len(personwiseKeypoints)):
                ...
                cv.line(fil, (B[0], A[0]), (B[1], A[1]), colors[i], 1, cv.LINE_AA)
