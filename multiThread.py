...
def ThreadViolent():
    for filenum in range(1,totalViolent):
        filename="/content/fight-detection-surv-dataset/fight/fi0"+str(filenum)+".mp4"
        ...
        while(True):
            ...
            if ret:
                ...
                if div==0 and counting<=extractedFrames:
                    ...
                    nam='./OpenPoseMatrices/v'+str(filenum)+'_'+str(counting) #save matrix files to a folder
...
# Filename = file location for each video, name is depending on dataset and id number
        filename="/content/fight-detection-surv-dataset/noFight/nofi0"+str(filenum)+".mp4"
        cam = cv2.VideoCapture(filename)
        print(filename)
        #Calculate total number of frames
        totalFrames = cam.get(cv2.CAP_PROP_FRAME_COUNT)
        ...
        while(True):
            ...
            if ret:
                ...
                if div==0 and counting<=extractedFrames:
                    # if video is still left continue creating images
                    print(str(counting)+' '+str(currentframe))
                    nam='./OpenPoseMatrices/nv'+str(filenum)+'_'+str(counting) #save matrix files to a folder
                    name  = './OpenPoseImages/holder.jpg'
                    counting = counting +1
                    #print ('Creating...' + name)
                    #scale_percent = 60 # percent of original size
        ...
        print("Execution time: "+str(exTime))
try:
	t1 = threading.Thread(target=ThreadViolent)
	t2 = threading.Thread(target=ThreadNoViolen)
	t1.start()
	t2.start()
	t1.join()
	t2.join()
